    ...
    public class WorkflowMapProfile : Profile
    {
        public WorkflowMapProfile()
        {
            ...
            CreateMap<Workflow, WorkflowOutputDto>()
                ...
                // Runtime Permission 1:1 mappings (JSONB string → List<string> for team lists)
                .ForMember(dest => dest.RuntimeUseSameAsTemplate, opt => opt.MapFrom(src => src.RuntimeUseSameAsTemplate))
            ...
            CreateMap<WorkflowInputDto, Workflow>()
                ...
                // Runtime Permission 1:1 mappings (List<string> → JSONB string for team lists)
                .ForMember(dest => dest.RuntimeUseSameAsTemplate, opt => opt.MapFrom(src => src.RuntimeUseSameAsTemplate))
                .ForMember(dest => dest.RuntimeViewPermissionMode, opt => opt.MapFrom(src => src.RuntimeViewPermissionMode))
