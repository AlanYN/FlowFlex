...
const filterNode = (value: string, data: FlowflexUser): boolean => {
	...
	if (!props.isShowAdminUser && nodeData.userType && nodeData.userType == UserType.SystemAdmin) {
		return false;
	}
...
const openModal = async () => {
	...
	if (props.beforeOpen) {
		try {
			...
			// 如果执行过程中出错，显示错误信息并阻止打开弹窗
			console.error('Error in beforeOpen hook:', error);
			ElMessage.error(error instanceof Error ? error.message : 'Failed to validate');
			return;
		}
	}
...
watch(
	...
	async (newData) => {
		if (newData !== undefined) {
			...
			// 如果传入了 undefined，清空数据并从缓存加载
			console.log('choosableTreeData is undefined, clearing data');
			treeData.value = [];
			rawTreeData.value = [];
			userDataMap.value.clear();
		}
