...
=======
		...
		/**
		 * 从localStorage刷新token状态
		 */
		async refreshTokenFromStorage() {
			const cachedTokenObj = getAuthCache<TokenObj>(TOKENOBJ_KEY);
			if (cachedTokenObj) {
				this.tokenObj = cachedTokenObj;
				console.log('[UserStore] Token refreshed from localStorage');
			} else {
		 ...
		 * 从localStorage刷新用户信息
		 */
		async refreshUserInfoFromStorage() {
			const cachedUserInfo = getAuthCache<UserInfo>(USER_INFO_KEY);
			if (cachedUserInfo) {
				this.userInfo = cachedUserInfo;
				console.log('[UserStore] UserInfo refreshed from localStorage');
			} else {
				this.userInfo = null;
				console.log('[UserStore] No userInfo found in localStorage');
			}

			// 同时刷新其他相关状态
			const cachedRoleList = getAuthCache<RoleEnum[]>(ROLES_KEY);
			if (cachedRoleList) {
				this.roleList = cachedRoleList;
			}

			const cachedIsLogin = getAuthCache<boolean>(ISLOGIN_KEY);
			if (cachedIsLogin !== undefined) {
				this.isLogin = cachedIsLogin;
			}
		},
>>>>>>> Stashed changes
	},
