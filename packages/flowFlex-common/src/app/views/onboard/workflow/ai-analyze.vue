<template>
  <div class="ai-analyze-page">
    <div class="header">
      ...
      <p class="subtitle">Upload files, analyze with AI, preview structured data, and import into Workflow</p>
    </div>
    ...
    <div v-show="step === 0" class="card">
      ...
      >
        ...
        <div class="el-upload__tip text-xs">Supported: .txt, .md, .csv, .tsv, .xlsx, .xls</div>
      </el-upload>
      <div class="actions">
    ...
    <div v-show="step === 1" class="card">
      <div class="flex items-center gap-2 mb-3">
        ...
        <el-select v-model="modelId" placeholder="Model from config" clearable style="width: 260px" @change="onModelIdChange">
          <el-option v-for="m in modelOptions" :key="m.id" :label="mDisplay(m)" :value="String(m.id)" />
        </el-select>
        <el-input v-model="modelProvider" placeholder="Provider (override)" style="width: 160px" />
        <el-input v-model="modelName" placeholder="Model (override)" style="width: 200px" />
        <el-tooltip :content="defaultModelTip" placement="top">
          <el-tag type="info" effect="plain">Default: {{ aiDefaultProvider || '-' }}/{{ aiDefaultModel || '-' }}</el-tag>
        </el-tooltip>
        <el-button @click="showModelConfig = true" type="success" plain>AI Model Config</el-button>
      </div>
      <div class="flex items-center gap-2 mb-3">
        <el-input v-model="promptOverride" placeholder="Optional: Brief description to guide AI" />
        <el-button type="primary" :loading="analyzing" @click="analyze">Analyze</el-button>
      </div>
      <el-input type="textarea" :rows="12" v-model="rawText" placeholder="Preview file content" />
      <div class="actions">
    ...
    <div v-show="step === 2" class="card">
      <el-card shadow="hover" class="mb-3">
        ...
        <div>
          <div class="mb-2"><strong>Name:</strong> {{ workflowPreview.name }}</div>
          <div class="mb-2"><strong>Description:</strong> {{ workflowPreview.description }}</div>
          <div class="mb-2"><strong>Stages:</strong> {{ stagePreview.length }}</div>
        </div>
      </el-card>
      ...
      <el-table :data="stagePreview" border height="420">
        ...
        <el-table-column label="Required Fields" min-width="260">
          <template #default="{ row }">
            <el-tag v-for="f in (row.requiredFields||[])" :key="f" class="mr-1 mb-1">{{ f }}</el-tag>
          </template>
        </el-table-column>
          ...
          <template #default="{ row }">
            <template v-if="row.checklistText && row.checklistText.trim()">
              <el-tooltip placement="top" :content="row.checklistText">
                <span class="text-gray-700 truncate block max-w-[200px]">{{ row.checklistText }}</span>
              </el-tooltip>
            </template>
            <template v-else>
              <el-tag v-if="row.checklistCount > 0" type="success">Existing ({{ row.checklistCount }})</el-tag>
          ...
          <template #default="{ row }">
            <template v-if="row.questionnaireText && row.questionnaireText.trim()">
              <el-tooltip placement="top" :content="row.questionnaireText">
                <span class="text-gray-700 truncate block max-w-[220px]">{{ row.questionnaireText }}</span>
              </el-tooltip>
            </template>
            <template v-else>
              <el-tag v-if="row.questionnaireCount > 0" type="success">Existing ({{ row.questionnaireCount }})</el-tag>
              <el-tag v-else type="info">Create</el-tag>
            </template>
          </template>
        </el-table-column>
    ...
    <div v-show="step === 3" class="card">
      <el-result icon="success" title="Import completed" sub-title="You can review the created workflow in Workflow page." />
      <div class="actions">
  ...
  <el-dialog v-model="showModelConfig" title="AI Model Configuration" width="60%" :close-on-click-modal="false">
    <AIModelConfig />
    <template #footer>
...
import { parseAIRequirements, generateAIWorkflow, getAIWorkflowStatus, getAIModels } from '@/apis/ai/workflow';
    ...
    if (name.endsWith('.xlsx') || name.endsWith('.xls')) {
      ...
      rows.slice(1).forEach((r) => {
        ...
        if (checklistCell || questionnaireCell) {
          extras[stageName] = {
            checklistText: checklistCell && checklistCell !== '--' ? checklistCell : undefined,
            questionnaireText: questionnaireCell && questionnaireCell !== '--' ? questionnaireCell : undefined,
          };
        }
      });
...
const analyze = async () => {
  ...
  try {
    ...
    const payload: any = {
      description: promptOverride.value?.trim() ? `${promptOverride.value}\n\n${description}` : description,
    };
    ...
    workflowPreview.value = data.generatedWorkflow || { name: 'AI Generated Workflow', description: 'Auto-created by AI' };
    stagePreview.value = (data.stages || []).map((s: any, idx: number) => ({
      ...
      checklistText: stageExtraMap.value[(s?.name || '').toString().trim()]?.checklistText || '',
      questionnaireText: stageExtraMap.value[(s?.name || '').toString().trim()]?.questionnaireText || '',
    }));
    ...
    for (const s of stagePreview.value) {
      ...
      try {
        await createChecklist({
          ...
          description: (s as any).checklistText || s.description || 'Auto-generated by AI',
      ...
      try {
        ...
        await createQuestionnaire({
          name: qn,
          description: (s as any).questionnaireText || s.description || 'Auto-generated by AI',
          status: 'Draft',
      ...
      const staticFields: string[] = Array.isArray(s.requiredFields)
        ? s.requiredFields.map((x: string) => (x || '').toUpperCase().replace(/\s+/g, '')).filter((x: string) => !!x)
        : [];
      if (staticFields.length > 0) {
        try {
          await updateStage(target.id, {
            ...
            estimatedDuration: target.estimatedDays || target.estimatedDuration || s.estimatedDuration || 1,
            visibleInPortal: true,
            attachmentManagementNeeded: false,
            components: [
              {
