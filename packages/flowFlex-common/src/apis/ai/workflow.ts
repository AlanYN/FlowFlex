...
// API Functions
export const generateAIWorkflow = (prompt: string): Promise<ApiResponse<AIWorkflowGenerationResult>> => {
  return defHttp.post({
...
export const enhanceAIWorkflow = (workflowId: number, enhancementRequest: string): Promise<ApiResponse<AIWorkflowEnhancementResult>> => {
  return defHttp.post({
...
export const validateAIWorkflow = (workflow: AIWorkflow): Promise<ApiResponse<AIWorkflowValidationResult>> => {
  return defHttp.post({
...
export const chatWithAI = (message: string, conversationId?: string): Promise<ApiResponse<{ response: string; conversationId: string }>> => {
  return defHttp.post({
    url: '/api/ai/chat',
    data: { 
      message,
