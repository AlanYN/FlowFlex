...
- [x] 1. Phase 1：后端数据模型与 DTO
  - [x] 1.1 确认 `Stage.cs` 上 `ComponentWeights` 属性已存在（无需修改）
  ...
  - [x] 1.2 新建 `ComponentWeightItem.cs` DTO
  ...
  - [x] 1.3 `StageInputDto.cs` 新增 `ComponentWeights` 字段
...
- [x] 2. Phase 2：后端业务逻辑
  - [x] 2.1 `StageService.UpdateAsync`：校验 WeightSum == 100
  ...
  - [x] 2.2 `StageService.UpdateAsync`：事务块内序列化 ComponentWeights
  ...
  - [x] 2.3 `GanttService.ComputeCompletionPercentage`：完善加权计算与孤立记录过滤
...
- [x] 4. Phase 4：前端类型定义
  - [x] 4.1 `types/onboard.d.ts` 新增 `ComponentWeightItem` 接口，扩展 `ComponentsData`
...
- [x] 5. Phase 5：前端 StageComponentsSelector 核心改造
  - [x] 5.1 新增 `componentWeights` 响应式状态和工具函数
  ...
  - [x] 5.4 watch `props.modelValue.componentWeights`，加载已保存数据或触发 fallback
  ...
  - [x] 5.5 实现 `balanceWeights`（均分非 QuickLink，余数补第一个）
    - 新增函数 `balanceWeights(): void`，逻辑与 `applyFallbackWeights` 相同（均分非 quickLink），完成后调用 `emitWeights()`
    - _Requirements: 2.2_

  - [x] 5.6 实现 `emitWeights`（拼装 `ComponentWeightItem[]` 并 emit）
  ...
  - [x] 5.7 WeightEditor 模板：每个 SelectedItem 卡片内新增 Slider + 数字输入框行
    - 在现有每个 SelectedItem 渲染卡片内（Portal Access 行之前），添加：
      ...
      <div class="border-t px-3 py-2 flex items-center gap-3">
        ...
        <el-slider
          ...
          class="flex-1"
          size="small"
          @input="(v) => setWeight(element.type, element.key, Number(v))"
        />
        <el-input-number
          :model-value="getWeight(element.type, element.key)"
          :min="0"
  ...
  - [x] 5.8 Balance 按钮：SelectedItems 区域右上角，空列表时禁用
  ...
  - [x] 5.9 联动：新增 Component 时权重初始化为 0，删除时从 Map 移除
  ...
  - [x] 5.10 TotalWeightIndicator：底部合计行，绿色/橙色状态
    - 新增 `computed` 属性：
  ...
  - [x] 5.11 PreviewPanel：以 100% 假设完成度展示每行公式，含进度条
  ...
  - [x] 5.12 Checkpoint — 前端开发服务器可正常运行，选中 Component 时 WeightEditor 正确渲染
    - 确保所有 TypeScript 无类型错误，询问用户是否有疑问。

- [x] 6. Phase 6：前端 StageForm 集成
  ...
  - [ ] 6.4 `submitForm` 前端校验（WeightSum≠100 时报错阻断）
    - 在现有权限校验之后、API 调用之前插入：
      ...
      if (formData.value.components.length > 0) {
        ...
        const hasNonQuickLink = formData.value.componentWeights.some(
          (w) => w.type !== "quickLink",
        );
...
- [x] 7. Phase 7：前端 API 类型更新
  - [x] 7.1 `apis/ow/index.ts` Stage Update 请求体和 Stage 查询响应体新增 `componentWeights`
